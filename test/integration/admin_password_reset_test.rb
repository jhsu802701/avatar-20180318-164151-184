...
class AdminPasswordResetTest < ActionDispatch::IntegrationTest
  def password_reset(user1, email1, password1)
    ...
    open_email(email1)
    assert current_email.subject.include?('Temporary Avatar App: Reset password instructions')
    assert current_email.body.include?('message from Temporary Avatar App')
